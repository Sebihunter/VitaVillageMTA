...
addEventHandler( "onClientRender", getRootElement(),
	function(  )
		local entryInteriors = getElementsByType ( "interiorEntry", getRootElement() )
		for key, v in pairs (entryInteriors) do		
			local x, y, z = getElementPosition( v )
			local x1, y1, z1 = getElementPosition( getLocalPlayer() )
			z = z + 0.82
			local intdim =  tonumber(getElementData ( v, "dimension" ))
			local dim = getElementDimension( getLocalPlayer() )
			if (getDistanceBetweenPoints3D ( x, y, z, x1, y1, z1 ) <= 10) and (intdim == dim) and isLineOfSightClear(x, y, z, x1,y1,z1, true, false, false) == true then
				local X, Y = getScreenFromWorldPosition( x, y, z )
				if X then
					dxDrawText( getInteriorName(v),  X - 30+2, Y - 50+2, X + 30, Y+50, tocolor( 0,0,0, 255 ), 0.4, "bankgothic", "center", "center", false, false, false)
					dxDrawText( getInteriorName(v),  X - 30, Y - 50, X + 30, Y+50, tocolor( 255, 255, 255, 255 ), 0.4, "bankgothic", "center", "center", false, false, false) 
				end
			end
		end
	end	
