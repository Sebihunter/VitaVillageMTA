...
local function updateIndicatorState ( state )
	...
	if state.left then
		if not state.coronaLeft then
			...
			attachElements ( state.markerLeft[1], state.vehicle, xmin,  ymax, zmin )
			attachElements ( state.markerLeft[2], state.vehicle, xmin, -ymax, zmin )
		end
	...
	if state.right then
		if not state.coronaRight then
			...
			attachElements ( state.markerRight[1], state.vehicle,  -xmin,  ymax, zmin )
			attachElements ( state.markerRight[2], state.vehicle, -xmin, -ymax, zmin )
		end
		numberOfIndicators = numberOfIndicators + 1
...
local function processIndicators ( state )
	...
	if INDICATOR_TYPE == 'both' then 
		...
		if getElementData(state.vehicle, "motor") == true then
			ison = true 
		else
			ison = false
		end
	...
	if state.nextChange <= state.timeElapsed then
		...
		if state.currentState then
			...
			if playerVehicle == state.vehicle and getElementData(state.vehicle, "battery") ~= 0 then 		
				if ison == true then
		...
		if getVehicleType(v) == "Automobile" or getVehicleType(v) == "Bike" or getVehicleType(v) == "Quad" and getElementData(v, "battery") ~=	0 then
			...
			if getVehicleOccupant(v, 0) == localPlayer then
				...
				if indicator == 'left' then
					if getElementData(v, "motor") == true then
